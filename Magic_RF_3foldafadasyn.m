%% Random Forests for normalised and balanced data after ADASYN is performed on the dataset
...
RF_summary = table('Size',[0 11],'VariableTypes',varTypes);
%add headers for the table
RF_headers = {'Parameters' 'oob_Errors','Accuracy','class_error',...
                'TrainTime','TPR','TNR',...
                'PPV','NPV','F1','AUC'};
RF_summary.Properties.VariableNames = RF_headers;

%create a new table with the same headers 
RF_folds_summary = RF_summary;
...
for i = 1:length(NumTree)
    ...
    for j = 1:length(MinLeaf)
       ...
       %in an array 
            ...
            RF_summary = [Parameters(1,:) oob_Errors_mean Accuracy_mean class_error_mean...
                            TrainTime_mean TPR_mean TNR_mean PPV_mean NPV_mean F1_mean AUC_mean]
            RF_summary_table = [RF_summary_table; RF_summary];
            RF_folds_summary = [RF_folds_summary; Final];
       end
   end 
end

%% convert array to table
RF_summary_final = array2table(RF_summary_table,'VariableNames',{'NumTree', 'NumLeaves' ,'NumPredictors',...
                                  'oobError', 'Accuracy', 'class_error',...
                                  'TrainTime','TPR','TNR','PPV','NPV', 'F1', 'AUC'});

%% save the tables
save RF_summary_table.mat
save RF_folds_summary.mat
save RF_summary_final.mat
...
highestAccuracy = max(RF_summary_final{:,5})
best_model = RF_summary_final(RF_summary_final.Accuracy == highestAccuracy, :)
best_NumTree = best_model{:,1} ; 
best_MinLeaf = best_model{:,2};
best_NumPredictors = best_model{:,3} ;

%% Accuracy calculation using class error
best_accuracy_new = 1 - best_model{1,6}

%% Add it to the best model table
best_model.accuracy_final = [best_accuracy_new]

%% %% CREATING BEST MODEL FROM OPTIMISED PARAMETERS
net_RF_3fold_final = TreeBagger(best_NumTree,test_k(:,1:10), test_k(:,11),'OOBPrediction', 'on','Method','Classification', ...
            'MinLeafSize', best_MinLeaf,'NumPredictorsToSample', best_NumPredictors);
%saving the best model
save('net_RF_3fold_final.mat');

toc;

%% Plot Graphs
% Number of trees box plot with mean classification error
    fig1 = figure
    boxplot(table2array(RF_summary_final(:,6)),table2array(RF_summary_final(:,1)));
    title({'Classification error with differing','numbers of trees'})
    xlabel('Number of trees');
    ylabel({'mean classification error','across all models'})
    
   %% Numbers of predictors box plot with mean classification error
    
    boxplot(table2array(RF_summary_final(:,6)),table2array(RF_summary_final(:,3)))
    title({'Classification error with differing','numbers of predictors'})
    xlabel('Number of predictors');
    ylabel({'mean classification error','all models'})

 %% Minimum leaf size box plot with error
    boxplot(table2array(RF_summary_final(:,6)),table2array(RF_summary_final(:,2)))
    title({'Classification error with differing','minimum leaf size'})
    xlabel('minimum leaf size');   
    ylabel({'mean classification error','all models'})

    
%% Out-Of-Bag Classification chart.
figure
plot(oobError(net_RF_3fold_final))
xlabel('Number of Grown Trees')
ylabel('Out-of-Bag Classification Error')

%% train time vs number of trees
figure
boxplot(table2array(RF_summary_final(:,7)), table2array(RF_summary_final(:,1)))
title({'Train time with differing', 'number of trees'})
xlabel('Number of trees')
ylabel('Trian time across models')
